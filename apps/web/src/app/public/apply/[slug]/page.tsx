...
export default function ApplyPage({ params }: { params: { slug: string } }) {
  ...
  const orgName =
    (form as any)?.department?.name ||
    (form as any)?.departmentName ||
    'Graduate Admissions'
  const orgInitial = String(orgName || 'A').trim().charAt(0).toUpperCase() || 'A'
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 h-14 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 bg-gray-900 rounded flex items-center justify-center text-white text-xs font-bold">{orgInitial}</div>
          <div>
                ...
                <Fragment key={i}>
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 transition
                      ${i === step ? 'bg-gray-900 text-white' : i < step ? 'bg-green-500 text-white' : 'border border-gray-300 text-gray-400 bg-white'}`}
                  >
                    {i < step ? '✓' : i + 1}
                  </div>
                  {i < pages.length - 1 && (
      ...
      <main className="max-w-xl mx-auto px-4 py-8 pb-24">
        ...
        <h1 className="text-xl font-semibold mb-6 text-center">{pages.length > 1 ? `Step ${step + 1} of ${pages.length}` : 'Application Form'}</h1>
